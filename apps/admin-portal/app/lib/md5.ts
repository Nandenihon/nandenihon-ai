/**
 * Simple MD5 hash utility (browser + Node compatible)
 * Used for password hashing consistent with the database
 */
export function md5(str: string): string {
    // Using Node.js crypto if available
...
function md5Pure(str: string): string {
    ...
    const bytes = new Array(strUtf8.length);
    for (let i = 0; i < strUtf8.length; i++) {
        bytes[i] = strUtf8.charCodeAt(i);
    }

    const length8 = bytes.length;
    const length16 = length8 >> 2;
    const words: number[] = [];
    for (let i = 0; i < length16; i++) {
        words[i] = (bytes[i * 4] || 0) | ((bytes[i * 4 + 1] || 0) << 8) | ((bytes[i * 4 + 2] || 0) << 16) | ((bytes[i * 4 + 3] || 0) << 24);
    }
    const tail = length8 & 3;
    const tailByte = length8 - tail;
    words[tailByte >> 2] = [1, 256, 65536, 16777216][tail] | (words[tailByte >> 2] || 0);
    words[((length8 + 64) >> 9 << 4) + 14] = length8 * 8;
