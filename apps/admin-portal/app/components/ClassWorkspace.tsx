...
export default function ClassWorkspace({ classId, teacherMode = false }: { classId: number; teacherMode?: boolean }) {
    ...
    return (
        <div className="space-y-6">
            ...
            {tab === "roster" && <section className="rounded-2xl bg-white p-6 shadow-sm"><h2 className="text-xl font-bold">Roster aktif</h2><div className="mt-4 divide-y">{members.map((member) => <div key={member.id} className="flex justify-between py-4"><div><p className="font-bold">{member.name}</p><p className="text-sm text-neutral-50">{member.email}</p></div><div className="text-right"><span className="text-sm font-bold">{member.status}</span><p className="text-xs text-neutral-40">{new Date(member.joined_at).toLocaleDateString("id-ID", { timeZone: "Asia/Jakarta" })}</p></div></div>)}</div></section>}
        </div>
    );
}
