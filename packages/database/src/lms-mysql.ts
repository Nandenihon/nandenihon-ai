...
/**
 * Creates all LMS tables if they do not already exist.
 * All foreign keys reference the existing `users` table.
 * Uses utf8mb4 for full Japanese character support.
 *
 * Relationships:
 *   users (1) ──< (M) enrollments >── (M) courses  [Many-to-Many via enrollments]
 *   courses (1) ──< (M) lessons
 *   users   (1) ──< (M) lesson_progress
 *   lessons (1) ──< (M) lesson_progress
 *   users   (1) ──< (M) quiz_grades
 *   lessons (1) ──< (M) quiz_grades
 */
export async function ensureLmsTables(): Promise<void> {
    const ddl = `
        ...
        CREATE TABLE IF NOT EXISTS courses (
            ...
            level       ENUM('N5','N4','N3') NOT NULL,
            description TEXT          CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
            thumbnail   VARCHAR(500)  NULL
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
    `;

    const enrollmentsDdl = `
        -- 2. Enrollments (Many-to-Many bridge: users <-> courses)
    ...
    const lessonsDdl = `
        -- 3. Lessons (One-to-Many: courses -> lessons)
    ...
    const progressDdl = `
        -- 4. Lesson Progress (One-to-Many: users + lessons -> progress)
    ...
    const gradesDdl = `
        -- 5. Quiz Grades (One-to-Many: users + lessons -> grades)
    ...
    // Run sequentially to respect FK dependencies
    await queryMySQL(ddl);
    await queryMySQL(enrollmentsDdl);
    await queryMySQL(lessonsDdl);
    await queryMySQL(progressDdl);
    await queryMySQL(gradesDdl);
}
